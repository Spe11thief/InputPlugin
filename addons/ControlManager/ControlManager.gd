...
func init_maps():
	...
	for controller in config.controller_count + 1:
		for action in config.default_controller_map.actions:
			#run if keyboard + mouse
			if controller == 0:
				for i in range(action.keys.size()):
					commit_action_input_to_map(action, controller, INPUT_TYPE.KEY, i)
				for i in range(action.mouse_buttons.size()):
					commit_action_input_to_map(action, controller, INPUT_TYPE.MOUSE_BUTTON, i)
					
			#run if controller
			else:
				for i in range(action.joy_buttons.size()):
					commit_action_input_to_map(action, controller, INPUT_TYPE.JOY_BUTTON, i)
				for i in range(action.axii.size()):
					commit_action_input_to_map(action, controller, INPUT_TYPE.AXIS, i, action.axii[i].deadzone)
	print(InputMap.get_actions())
	
func build_input_name(controller: int, action_name: String, input_type: INPUT_TYPE, index: int):
	return "p" + str(controller) + "_" + action_name + "_" + input_dic[input_type] + "_" + str(index)

func commit_action_input_to_map(action: Action, controller: int, input_type: INPUT_TYPE, input_index: int, deadzone: float = 0):
	var input_name = build_input_name(controller, action.name, INPUT_TYPE.KEY, input_index)
...
func init_buffer():
	for controller in config.controller_count + 1:
		for action in config.default_controller_map.actions:
			buffer["p" + str(controller) + "_" + action.name] = [{"strength" : 0, "time": 0}]
...
func get_action_by_name(name: String):
	for action in config.default_controller_map.actions:
		if action.name.match(name):
			return action
	push_error("Action " + name + " does not exist")
	return null
...
func action_has_event(action_name: String, event: InputEvent, control_code: int = -1, control_map: Map = null):
	...
	var map = control_map
	if control_map == null:
		map = config.default_controller_map
	for controller in config.controller_count + 1:
...
func action_has_eventOLD(action_name: String, event: InputEvent):
	...
	var controller = event.device
	var action: Action = get_action_by_name(action_name)
	if controller == 0:
		for i in range(action.keys.size()):
			var godot_action_name = build_input_name(controller, action.name, INPUT_TYPE.KEY, i)
			if InputMap.action_has_event(godot_action_name, event):
				return true
		for i in range(action.mouse_buttons.size()):
			var godot_action_name = build_input_name(controller, action.name, INPUT_TYPE.MOUSE_BUTTON, i)
			if InputMap.action_has_event(godot_action_name, event):
				return true
	else:
		for i in range(action.joy_buttons.size()):
			var godot_action_name = build_input_name(controller, action.name, INPUT_TYPE.JOY_BUTTON, i)
			if InputMap.action_has_event(godot_action_name, event):
				return true
		for i in range(action.axii.size()):
			var godot_action_name = build_input_name(controller, action.name, INPUT_TYPE.AXIS, i)
			if InputMap.action_has_event(godot_action_name, event):
				return true
	return false
